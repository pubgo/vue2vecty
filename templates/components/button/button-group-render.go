...
func (t *_ButtonGroup) _Render() vecty.ComponentOrHTML {
	return elem.UnorderedList(vecty.Markup(vecty.Property("id", "example-1")), elem.Paragraph(vecty.Text(vecty.Text(func() string {
		if 0 == 0 {
			return "world"
		} else {
			return "hello"
		...
		for __item := range t.items {
			item := __item
			e = append(e, elem.ListItem(vecty.Markup(), vecty.Text(vecty.Text(item.message))))
		}
		return
	}(), func() (e vecty.List) {
		for __item, __index := range t.items {
			item, index := __item, __index
			e = append(e, elem.ListItem(vecty.Markup(), vecty.Text(vecty.Text(parentMessage+"-"+index+"-"+item.message))))
		}
		return
	}(), func() (e vecty.List) {
		for __value, __name := range t.object {
			value, name := __value, __name
			e = append(e, elem.Div(vecty.Markup(), vecty.Text(vecty.Text(index+"."+name+":"+value))))
		}
		return
	}(), func() (e vecty.List) {
		for __item := range t.items {
		...
		for __n := range t.evenNumbers {
			n := __n
			e = append(e, elem.ListItem(vecty.Markup(), vecty.Text(vecty.Text(n))))
		}
		return
	}(), vecty.If(!todo.isComplete, func() (e vecty.List) {
		for __todo := range t.todos {
			todo := __todo
			e = append(e, elem.ListItem(vecty.Markup(), vecty.Text(vecty.Text(todo))))
		}
		return
